...
//   - FOTO REAL DE LA SUCURSAL de fondo en TODAS las pantallas, con un
//     velo verde para legibilidad (y degradado de marca como respaldo)
...
//   + Pantalla de texto    -> respaldo para CUALQUIER respuesta sin
//     plantilla propia (garantiza que todo turno tenga pantalla)
...
// Fotografía real de la sucursal, de fondo en TODAS las pantallas
// (servida desde el sitio en Vercel, igual que el logo)
const FONDO = 'https://pier-reposteria.vercel.app/alexa-fondo.jpeg';
...
// Toda pantalla lleva capas fijas: la foto de la sucursal, un velo negro
// para que el texto respire, el contenido, y un FOOTER dorado persistente.
// Si la foto no carga, queda el degradado de marca como respaldo.
function baseDoc(contenido, hintFooter) {
  const capas = [
    {
      type: 'Image',
      source: FONDO,
...
function buildHeadline({ subtituloHeader, primario, secundario, hint }) {
  ...
  }, hint || 'Di "ayuda" para ver qué puedo hacer');
}

// ---------------------------------------------------------------------
// PANTALLA DE TEXTO - Respaldo universal: cualquier respuesta que no
// tenga plantilla propia muestra lo dicho, sobre la foto de la sucursal
// ---------------------------------------------------------------------
function buildPantallaTexto(texto, hint) {
  const recorte = String(texto || '').length > 220
    ? String(texto).slice(0, 217).trimEnd() + '…'
    : String(texto || '');
  return baseDoc({
    type: 'Container',
    width: '100vw',
    ...
    items: [
      headerPier(),
      {
        ...
        items: [
          {
            type: 'Frame',
            width: "${@viewportProfile == @hubRoundSmall ? '72dp' : '96dp'}",
            height: "${@viewportProfile == @hubRoundSmall ? '72dp' : '96dp'}",
            borderRadius: '48dp',
            backgroundColor: COLORES.crema,
            borderColor: COLORES.acento,
            borderWidth: '2dp',
          ...
          },
          {
            type: 'Text',
            text: recorte,
            fontSize: "${@viewportProfile == @hubRoundSmall ? '18dp' : '26dp'}",
            color: COLORES.crema,
            textAlign: 'center',
            paddingTop: '20dp',
            maxLines: 6,
...
function buildCardsCategorias(categorias) {
  ...
  }, 'Toca una categoría o dime cuál quieres ver');
}
...
// Para el PERSONAL la ficha es informativa: sin botón de compra, con el
// stock en línea a la vista y hints de operación en lugar de venta.
// ---------------------------------------------------------------------
function buildDetalleProducto(p, opciones) {
  const paraPersonal = !!(opciones && opciones.paraPersonal);
  ...
  const ratingTxt = p.rating && Number(p.rating) > 0
    ...
    : '';
  const stockTxt = paraPersonal && p.stock_online !== undefined && p.stock_online !== null
    ? ` En línea quedan ${Number(p.stock_online)} unidades.`
    : '';
  const doc = {
    type: 'AlexaDetail',
    detailType: 'generic',
    ...
    headerTitle: 'PIER REPOSTERÍA',
    headerSubtitle: paraPersonal ? 'Ficha del producto · Personal' : 'Detalle del producto',
    headerAttributionImage: LOGO,
    backgroundImageSource: FONDO,
    backgroundColorOverlay: true,
    ...
    bodyText: `${p.descripcion || 'Elaborado el mismo día, disponible para recoger en tienda.'}${ratingTxt}${stockTxt}`,
    theme: 'dark',
    width: '100vw',
    height: '100vh',
  };
  if (!paraPersonal) {
    doc.button1Text = 'Agregar al pedido';
    doc.button1PrimaryAction = {
      type: 'SendEvent',
      arguments: ['agregar', String(p.id || '')],
    };
  }
  return baseDoc(doc, paraPersonal
    ? 'Di: "repón diez unidades" o "se acabó" para el inventario'
    : 'Toca "Agregar al pedido" o dime: agrégalo');
}
...
function buildMultipleChoice(pregunta, opciones) {
  ...
  }, 'Toca una opción o dime: chico o grande');
}

// ---------------------------------------------------------------------
// 7. PANEL DE COMANDOS - Pantalla del PERSONAL: chuleta de frases de voz
// agrupadas por área (pedidos, ventas, entregas...). Se usa en la
// bienvenida y la ayuda cuando la sesión es de empleado/gerencia/dirección.
// comandos: [{ etiqueta, frase }]
// ---------------------------------------------------------------------
function buildPanelComandos(subtitulo, comandos, hint) {
  const data = (comandos || []).slice(0, 8).map(c => ({
    etiqueta: c.etiqueta || '',
    frase: c.frase || '',
  }));
  return baseDoc({
    ...
    height: '100vh',
    items: [
      headerPier(subtitulo || 'Panel de voz del personal'),
      {
        type: 'Sequence',
        grow: 1,
        width: '100%',
                ...
                items: [
                  {
                    type: 'Text',
                    text: '${data.etiqueta}',
                    fontSize: "${@viewportProfile == @hubRoundSmall ? '12dp' : '15dp'}",
                    color: COLORES.doradoClaro,
                    maxLines: 1,
                  },
                  {
                    ...
                    text: '${data.frase}',
                    fontFamily: FUENTE_TITULOS,
                    fontSize: "${@viewportProfile == @hubRoundSmall ? '16dp' : '21dp'}",
                    fontWeight: '700',
                    color: COLORES.crema,
                    maxLines: 2,
                  },
  ...
  }, hint || 'Dime cualquiera de estas frases');
}
