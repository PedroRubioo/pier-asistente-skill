...
function buildSystemPrompt(contextoTexto, horarioTexto, instruccionEspecifica, attrs, tienePantalla, fichaActiva) {
  ...
  const capacidadesPersonal = esPersonal ? `

COMO ES PERSONAL DE PIER, TAMBIÉN PUEDE OPERAR EL NEGOCIO POR VOZ (guíalo con estas frases):
...
- Reportes de dirección: «dame el resumen del mes», «qué movimientos hubo en el sistema».` : ''}` : '';
  ...
  return `Eres Pier, una empleada amigable y joven de Pier Repostería, una repostería artesanal en Huejutla de Reyes, Hidalgo. Hay dos modalidades: recoger en tienda, o envío a domicilio en colonias con cobertura. Eres la asistente para EXPLORAR por voz: catálogo, precios, recomendaciones y armar el carrito; la COMPRA (pago y confirmación del pedido) siempre se completa en la web. Si el usuario quiere finalizar su pedido, dile con naturalidad que su carrito ya lo espera en ${PIER_WEB} para confirmarlo y pagarlo.
...
- «Qué notificaciones tengo», «cuánto cuesta el envío a [colonia]», promociones, horario, ubicación.${capacidadesPersonal}
...
async function aplParaContexto(handlerInput) {
  ...
  if (attrs.productosListados && attrs.productosListados.length > 1) {
    ...
    return { doc: buildImageList('Lo que te mencioné', items, 'Toca uno para ver su detalle'), token: 'conversacionToken' };
  }
