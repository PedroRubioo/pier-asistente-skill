...
function buildSystemPrompt(contextoTexto, horarioTexto, instruccionEspecifica, attrs, tienePantalla, fichaActiva) {
  ...
  const esPersonal = attrs?.usuarioAutenticado
    && ['empleado', 'gerencia', 'direccion_general'].includes(attrs.usuarioAutenticado.rol);
...
- Entregas: «qué entregas hay», «qué repartidores están disponibles», «asigna el pedido [dígitos] a [nombre]», «avisa demora del pedido [dígitos]».` : '';
