...
const { soportaAPL, buildImageList, buildDetalleProducto } = require('./apl');
const { esPersonal: esSesionPersonal } = require('./auth');
...
function buildSystemPrompt(contextoTexto, horarioTexto, instruccionEspecifica, attrs, tienePantalla, fichaActiva) {
  ...
  const rolUsuario = attrs?.usuarioAutenticado?.rol;
  const esPersonal = ['empleado', 'gerencia', 'direccion_general'].includes(rolUsuario);
  const esGerenciaOMas = ['gerencia', 'direccion_general'].includes(rolUsuario);
  const esDireccion = rolUsuario === 'direccion_general';

  // Bloque de capacidades: al PERSONAL se le habla como colega y se le
  // guía a las operaciones; al cliente se le acompaña a explorar y comprar
  const bloqueCapacidades = esPersonal ? `ESTA SESIÓN ES DE PERSONAL DE PIER. Trátalo como COLEGA de trabajo, NO como cliente: PROHIBIDO venderle, ofrecerle apartar productos, sugerirle el carrito o los favoritos, o rematar con preguntas de venta tipo "¿te lo aparto?" o "¿lo quieres chico o grande?". Las funciones de compra (carrito, favoritos, pide lo de siempre, mis pedidos) son EXCLUSIVAS de las cuentas de cliente y NO existen en cuentas de personal: si las pide, díselo con amabilidad y recuérdale que para comprar necesita su cuenta personal de cliente.

LO QUE EL PERSONAL PUEDE HACER POR VOZ (si quiere algo de esto, dile la frase exacta):
- Pedidos: «cómo van los pedidos», «qué pedidos están pendientes», «qué lleva el pedido [últimos dígitos]», «marca el pedido [dígitos] como listo», «cancela el pedido [dígitos]».
- Ventas e inventario: «cómo van las ventas hoy», «qué está agotado», «repón N unidades de [producto]», «marca [producto] como agotado».
- Entregas: «qué entregas hay», «qué repartidores están disponibles», «asigna el pedido [dígitos] a [nombre]», «avisa demora del pedido [dígitos]».${esGerenciaOMas ? `
- Reportes de gerencia: «cuáles son los números del negocio», «cuáles son los productos más vendidos», «cómo van las ventas de la semana», «cuántos empleados tenemos».` : `
- Los reportes del negocio (números totales, ranking de ventas, semana, equipo) son de gerencia y dirección: si los pide, díselo con amabilidad.`}${esDireccion ? `
- Reportes de dirección: «dame el resumen del mes», «qué movimientos hubo en el sistema».` : ''}

DATOS OPERATIVOS (ventas, inventario, pedidos del negocio, entregas, reportes): TÚ NO LOS TIENES en este prompt y está PROHIBIDO inventarlos, estimarlos o dar cifras de memoria. Si te pregunta por alguno, responde ÚNICAMENTE indicándole la frase exacta que debe decir (por ejemplo: "dime: cómo van las ventas hoy, y te doy el dato real"). El CATÁLOGO con precios sí lo tienes y sí puedes responderlo.` : `LO QUE PUEDES HACER POR EL USUARIO EN ESTA CONVERSACIÓN (si quiere algo de esto, dile la frase exacta):
- Carrito: «agrega X al carrito», «quita X del carrito», «qué tengo en mi carrito», «vacía mi carrito». (La compra se confirma en la web.)
...
- «Qué notificaciones tengo», «cuánto cuesta el envío a [colonia]», promociones, horario, ubicación.`;
  ...
  const bloquePersonalidad = esPersonal ? `PERSONALIDAD Y TONO (sesión de personal):
- Habla como compañera de equipo detrás del mostrador: directa, cálida y breve. Cero tono de venta.
...
- Usa su nombre de vez en cuando (no en cada turno).
- Si te preguntan algo a lo que ya respondiste, contesta con naturalidad y avanza.` : `PERSONALIDAD Y TONO:
...
- Si te preguntan algo a lo que ya respondiste, contesta con naturalidad: "ya te decía que..." o resume y avanza.`;
  ...
  return `Eres Pier, una empleada amigable y joven de Pier Repostería, una repostería artesanal en Huejutla de Reyes, Hidalgo. Hay dos modalidades: recoger en tienda, o envío a domicilio en colonias con cobertura. ${esPersonal
    ? 'En esta sesión eres el PANEL DE VOZ del personal de la tienda: apoyas con consultas del catálogo y guías hacia las operaciones del negocio.'
    : `Eres la asistente para EXPLORAR por voz: catálogo, precios, recomendaciones y armar el carrito; la COMPRA (pago y confirmación del pedido) siempre se completa en la web. Si el usuario quiere finalizar su pedido, dile con naturalidad que su carrito ya lo espera en ${PIER_WEB} para confirmarlo y pagarlo.`}
...
// Pantalla para respuestas CONVERSACIONALES de la IA: si en el turno se
// mencionaron varios productos → lista con fotos; si uno → su ficha con
// botón de agregar. Así lo que Alexa dice SIEMPRE se ve en pantalla.
// Llamar DESPUÉS de responderConIA (que actualiza el estado detectado).
async function aplParaContexto(handlerInput) {
  const attrs = handlerInput.attributesManager.getSessionAttributes();
  const paraPersonal = esSesionPersonal(handlerInput);
  const completo = await obtenerCatalogoCompleto().catch(() => []);
  const porId = (id) => completo.find(p => p.id === id);

  if (attrs.productosListados && attrs.productosListados.length > 1) {
    const items = attrs.productosListados.slice(0, 6).map(pl => {
      const full = porId(pl.id) || pl;
      return {
        primario: full.nombre,
        secundario: '$' + Number(full.precio_chico || 0).toFixed(0) + ' MXN',
        terciario: full.categoria || '',
        imagen: full.imagen_url || '',
        id: full.id,
      };
    });
    return { doc: buildImageList('Lo que te mencioné', items, paraPersonal ? 'Toca uno para ver su ficha' : 'Toca uno para ver su detalle'), token: 'conversacionToken' };
  }

  const pa = attrs.productoActivo;
  if (pa && pa.id) {
    const full = porId(pa.id);
    return {
      doc: buildDetalleProducto({
        ...
        rating: full?.rating,
        stock_online: full?.stock_online,
      }, { paraPersonal }),
      token: 'productoToken',
    };
  }
...
module.exports = { responderConIA, detectarProductosMencionados, aplParaContexto };
